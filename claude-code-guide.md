...
## 21. 🟡 Iterative Self-Refinement — автономное улучшение кода за ночь
...
/loop 50 "Проанализируй код проекта. Найди одну проблему (баг, дубликат, 
          ...
          Запиши в ITERATIONS.md: что нашёл, что исправил, почему."
```

Способ 2 — через `Stop` hook (агент не останавливается пока есть что улучшать):
```json
// .claude/settings.json
{
  "hooks": {
...
echo "Готово. Посмотри git log и ITERATIONS.md"
```
...
git diff main      # весь diff для ревью
```

**Результат:** код стал чище, быстрее, безопаснее. Каждое изменение задокументировано. Можно откатить любую итерацию отдельно.

**⚠️ Обязательные меры безопасности:**

| Что | Зачем |
|-----|-------|
| `git commit` перед запуском | Откат если всё сломалось |
...
**⚠️ Расход токенов:** 50 итераций на проекте 15k строк ≈ серьёзный расход. Считай бюджет заранее.
